...
import { ShopInfo } from '../lib/billing';
...
export const Dashboard: React.FC<DashboardProps> = ({ session, setCurrentView, currentView, shop }) => {
  ...
  return (
    <div className="min-h-[calc(100vh-72px)] pt-[72px]">
      ...
      {/* Main Content */}
      <main className="lg:ml-64 bg-slate-50 min-h-full">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-[72px] z-10 shadow-sm">
           <div>
           ...
           <div className="flex items-center gap-2 sm:gap-4">
              <div className="hidden sm:flex flex-col items-end mr-4">
                  ...
                  <span className="text-sm sm:text-lg font-bold text-emerald-600">+$248.50 today</span>
              </div>
              <button
                className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-slate-500 hover:text-blue-600 hover:bg-slate-50 transition-colors relative focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                aria-label="Notifications"
              >
                 <Bell size={20} />
                 <span className="absolute top-2 right-2.5 w-2 h-2 rounded-full bg-red-500 border border-white" aria-hidden="true"></span>
              </button>
           </div>
        </header>
        ...
        <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
             ...
             <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                  <Button onClick={() => setCurrentView('upload-files')} className="gap-2 w-full sm:w-auto justify-center sm:justify-start">
                    <UploadCloud size={16} />
                    Upload RO
                  </Button>
                  <Button variant="outline" onClick={() => setCurrentView('ro-audit')} className="gap-2 w-full sm:w-auto justify-center sm:justify-start">
                    <Compass size={16} />
                    Audit RO
                  </Button>
                </div>
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
                 <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="text-2xl sm:text-3xl font-bold text-slate-900">$2,840.50</div>
                    <div className="text-emerald-600 text-sm font-medium mt-2 flex items-center gap-1">
                        <ArrowUpRight size={14} aria-hidden="true" /> +12% vs last month
                    </div>
                 </div>
                 <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm">
                    <div className="text-slate-500 text-sm font-medium mb-1">Parts Audited</div>
                    <div className="text-2xl sm:text-3xl font-bold text-slate-900">482</div>
                    <div className="text-blue-600 text-sm font-medium mt-2">
                        100% Coverage
                    </div>
                 </div>
                 <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm sm:col-span-2 lg:col-span-1">
                    <div className="text-slate-500 text-sm font-medium mb-1">Active Slippage</div>
                    <div className="text-2xl sm:text-3xl font-bold text-slate-900">2.4%</div>
                    <div className="text-emerald-600 text-sm font-medium mt-2 flex items-center gap-1">
                        <Check size={14} aria-hidden="true" /> Target achieved
                    </div>
                 </div>
             </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
                <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6 lg:col-span-2">
                  ...
                  <div className="h-32 sm:h-48 rounded-lg bg-gradient-to-b from-slate-50 to-white border border-dashed border-slate-200 flex items-center justify-center text-slate-400 text-sm">
             ...
             <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-8">
                ...
                <div className="divide-y divide-slate-100">
                  {[
                    ...
                    <div key={row.vendor} className="px-4 sm:px-6 py-3 grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 text-sm text-slate-800">
                      <div className="font-semibold text-slate-900 sm:col-span-1">{row.vendor}</div>
                      <div className={`${row.color} sm:col-span-1`}>{row.savings} avg</div>
                      <div className="text-slate-700 sm:col-span-1">{row.fill} fill</div>
                      <div className="text-slate-500 sm:col-span-1">{row.eta} avg ETA</div>
                    </div>
                  ))}
                </div>
             </div>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
               ...
               <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4 sm:p-6 space-y-4">
                 <div className="flex items-start sm:items-center justify-between">
                   ...
                   <Button variant="outline" size="sm" className="mt-2 sm:mt-0">View all</Button>
                 </div>
                 <div className="divide-y divide-slate-100">
                   {[
                     ...
                     <div key={item.ro} className="py-3 flex items-center justify-between">
                       <div className="min-w-0 flex-1">
                         ...
                         <div className="text-xs text-slate-500 truncate">{item.vehicle}</div>
                       </div>
                       <div className="text-sm font-bold text-emerald-700 ml-4">{item.savings}</div>
                     </div>
                   ))}
                 </div>
               </div>
